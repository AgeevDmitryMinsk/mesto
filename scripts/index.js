...
console.log(templateElement.textContent)
...
console.log(profilePopup.textContent)
...
console.log(inputLink.textContent)
...
function  openImagePopup(evt)  { // объявляем функцию, реализующую открытие попап-окна для просмотра выбранного изображения XXL; стрелочную переделал в обычную для this
...
function handleAddElementFormSubmit(evt) { // объявляем функцию, реализующую сохранение значений полей ввода данных и создание новой карточки;
  ...
  cardsContainer.prepend(newElement); // вставляем новую карточку в начало списка; append - в конец
